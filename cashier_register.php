...
// Restrict access (example: only allow if admin is logged in or via secret URL)
// For now, we assume this page is accessed by an admin; you can add proper auth later
    ...
    if (empty($errors)) {
        ...
        try {
            $user_query = "INSERT INTO tbluser (fname, lname, username, password, role) 
                          VALUES ('$fname', '$lname', '$username', '$hashed_password', 'cashier')";
            if (mysqli_query($connection, $user_query)) {
                $uid = mysqli_insert_id($connection);
                $cashier_query = "INSERT INTO tblcashier (uid, timeIn, timeOut) VALUES ($uid, '0000-00-00 00:00:00', '0000-00-00 00:00:00')";
                if (mysqli_query($connection, $cashier_query)) {
                    mysqli_commit($connection);
                    $success = true;
                } else {
                    throw new Exception("Error creating cashier account");
                }
            }
        } catch (Exception $e) {
...
<head>
    ...
    <title>ChowNow - Register Cashier</title>
...
<body>
    <div class="register-container">
        ...
        <?php if (!$success): ?>
            <form method="post" action="" id="cashierRegisterForm">
                <div class="form-row">
                ...
                <button type="submit" name="register" class="btn-primary">Register Cashier</button>
            </form>
        <?php endif; ?>
